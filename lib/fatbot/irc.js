...
function IRCConnection(app, options) { 
    options = options || {};
    ...
    server.Server.call(this, app, options);
...
IRCConnection.prototype.connect = function() { 
    ...
    this.connection.addListener('connect', function() {
        ...
        self.emit('connect');
        self.app.emit('serverconnect', self);
    });
...
IRCConnection.prototype._send = function(msg) { 
    if( this._send_timer !== null ) {
        if( this._send_buffer.length > 100 ) {
            log('OOPS! more than 100 messages in send buffer - dropping messages!');
            log('OOPS! things might get confusing from now...');
