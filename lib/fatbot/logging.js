...
function Logger(options) {
    ...
    //}
    options.name = (options.name === undefined) ? ("logger"+(logger_c++)) : options.name;
...
Logger.prototype.exception = function() {
    ...
    var exc = args.shift();
    this.log(levels.error, args.join(" ")+' '+exc.toString()+"\n"+sys.inspect(exc.stack));
};
...
function ConsoleHandler(logger, formatter) { 
    ...
    logger.addListener('item', listener);

    this.remove = function() {
        logger.removeListener('item', listener);  
    };
}
